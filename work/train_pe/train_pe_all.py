...
def c_bulk_diamond2():
    ...
    test_error = [0.3120693448873019, 0.2621399410847088, 0.20526243745080078, 0.21793777501562317, 0.23144284835505285, 0.33300275645022953, 0.27705989836286077, 0.20853523156062542, 0.22163665160286744, 0.320720795102066, 0.3631611482960824, 0.2675228527981126, 0.19985273349578012, 0.26027539997099747, 0.3185054740111417, 0.3823029406564499, 0.2606881795774601, 0.23527867456890872, 0.3526077229227908, 0.6565668051523, 0.47376232492079945, 0.2419661185669313, 0.43795035705177054, 0.6285121359171832, 0.8901745773939764]
...
def train_si_dia():
    ...
    for ii, param in enumerate(param_list):
        lr, onsite_lr = param
        param = {'ml': {'lr': lr, 'onsite_lr': onsite_lr}}
        mae0 = train_function(dataset, ['si_bulk_diamond2'], [0.4], batch_size, train_onsite,
                              orbital_resolved, scale_ham, _params=param)
        train_error.append(mae0)

        os.system(f'mv opt.pkl opt-si_bulk_diamond2-{lr:f}-{onsite_lr:f}.pkl')

        err0, err1, _ = pred([f'opt-si_bulk_diamond2-{lr:f}-{onsite_lr:f}.pkl'], dataset, ['si_bulk_diamond2'], [0.2], batch_size)
        test_error.append([err0.numpy(), err1.numpy()])

    print(train_error, test_error)
    # lr = 0.006, onsite_lr = 0.003
    # train_error = [0.3918410274900406, 0.3085489878010935, 0.2793587272825925, 0.2673041915693579, 0.24354390452681085,
    #  0.3779144581288874, 0.3709989793321706, 0.3251253433591203, 0.3246981114836222, 0.3243097059032456,
    #  0.40032065086375895, 0.34426896917682565, 0.21908656413812005, 0.21924343080420772, 0.260479856200651,
    #  0.3471293224334383, 0.3930448338211222, 0.3863590257205155, 0.3355697597660756, 0.3005970030940054,
    #  0.3501206744352157, 0.490628798579354, 0.5283377673562624, 0.47526205509722497, 0.25969068386748145]

    test_err = [i[0] for i in test_error]
    plt.plot(np.arange(len(train_error)), train_error)
    plt.plot(np.arange(len(test_err)), test_err)
    plt.show()
